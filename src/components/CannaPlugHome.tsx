...
function Products() {
  ...
  const { add } = useCart();
  return <section className="page-section product-section" id="shop"><Reveal><SectionHeading eyebrow="Curated selection" title="FEATURED PRODUCTS" action="Shop all"/><div className="product-grid">{products.map(([name, cat, price, badge, pos]) => <article className="product-card" key={name}><div className="product-visual"><span className="badge">{badge}</span><img src={productImage} alt={name} className={pos} loading="lazy" width={1920} height={768}/></div><div className="product-info"><p>{cat}</p><h3>{name}</h3><div><b>{price}</b><Button aria-label={`Add ${name} to cart`} onClick={() => { add({ productId: `featured-${String(name)}`, name: String(name), price: Number(String(price).replace(/[^0-9.]/g, "")) || 0, unit: null }); setAdded(name ?? null); window.setTimeout(() => setAdded(null), 1600); }}>{added === name ? <><Check size={15}/> Added</> : <>Add to cart <ShoppingBag size={15}/></>}</Button></div></div></article>)}</div></Reveal></section>
}
...
function FloatingBottomNav() { return <motion.nav className="floating-nav" aria-label="Quick navigation" initial={{ y: 80, opacity:0 }} animate={{ y:0, opacity:1 }} transition={{ delay:.8, type:"spring" }}><Link to="/"><Store/><span>Home</span></Link><Link to="/shop"><ShoppingBag/><span>Shop</span></Link><Link to="/" className="float-leaf" aria-label="CannaPlug home"><Leaf fill="currentColor"/></Link><a href="/#events"><CalendarDays/><span>Events</span></a><a href="/#newsroom" className="desktop-float"><Newspaper/><span>Newsroom</span></a><Link to="/account" className="mobile-float"><CircleUserRound/><span>Account</span></Link></motion.nav> }
