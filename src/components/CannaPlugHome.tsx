...
function Products() {
  ...
  const { add } = useCart();
  return <section className="page-section product-section" id="shop"><Reveal><SectionHeading eyebrow="Curated selection" title="FEATURED PRODUCTS" action="Shop all"/><div className="product-grid">{products.map(([name, cat, price, badge, pos]) => <article className="product-card" key={name}><div className="product-visual"><span className="badge">{badge}</span><img src={productImage} alt={name} className={pos} loading="lazy" width={1920} height={768}/></div><div className="product-info"><p>{cat}</p><h3>{name}</h3><div><b>{price}</b><Button aria-label={`Add ${name} to cart`} onClick={() => { add({ productId: `featured-${name}`, name, price: Number(String(price).replace(/[^0-9.]/g, "")) || 0, unit: null }); setAdded(name ?? null); window.setTimeout(() => setAdded(null), 1600); }}>{added === name ? <><Check size={15}/> Added</> : <>Add to cart <ShoppingBag size={15}/></>}</Button></div></div></article>)}</div></Reveal></section>
}
